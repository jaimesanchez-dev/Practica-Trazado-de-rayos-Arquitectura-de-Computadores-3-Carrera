...
TEST(VentanaModeloMatematico, CalculoVectoresU_V) {
  ...
  vector vf = POV.resta(DEST).normalizar();
  vector u  = NORTE.producto_vectorial(vf).normalizar();
  vector v  = vf.producto_vectorial(u);
...
TEST(VentanaModeloMatematico, CalculoVectoresPH_PV) {
  ...
  vector DEST(0.0, 0.0, -10.0);
  vector NORTE(0.0, 1.0, 0.0);
  geometria geo(POV, DEST, NORTE, 90.0);

  tamaño_imagen tam(800, 600);
  mersenne_twister mt(0);

  double df = 10.0;
  double hp = 2.0 * std::tan((90.0 * M_PI / 180.0) / 2.0) * df;  // = 20
  double wp = hp * (800.0 / 600.0);                              // = 26.6667

  vector vf = POV.resta(DEST).normalizar();               // (0,0,1)
  vector u  = NORTE.producto_vectorial(vf).normalizar();  // (1,0,0)
  vector v  = vf.producto_vectorial(u);                   // (0,1,0)

  vector p_h = u.producto_constante(wp);
  vector p_v = v.producto_constante(-hp);
...
TEST(VentanaModeloMatematico, CalculoOrigenVentana) {
  ...
  vector vf     = POV.resta(DEST);
  vector vf_hat = vf.normalizar();
  vector u      = NORTE.producto_vectorial(vf_hat).normalizar();
  vector v      = vf_hat.producto_vectorial(u);

  vector p_h = u.producto_constante(wp);
  vector p_v = v.producto_constante(-hp);
  vector dx  = p_h.producto_constante(1.0 / 800.0);
  vector dy  = p_v.producto_constante(1.0 / 600.0);

  vector O = POV.resta(vf)
...
TEST(VentanaModeloMatematico, GeneracionRayoDesdePixel) {
  ...
  tamaño_imagen tam(400, 300);
  mersenne_twister mt(42);
  ventana v(geo, tam, mt);

  // Rayo en el centro de la ventana
  rayo r     = v.generar_rayos_pixel(200, 150);
  vector dir = r.obtener_direccion();
